...
fid= fopen([fusionPath,'sampleLog_LTP.txt'],'w');
ME_info = importdata([fusionPath,'MEinterval_info_',num2str(set_inteval),'_',num2str(shortLength2delete),trainVecComposition,'.mat']);
            ...
            for mm = 1:nb_gt
               ...
               % ---------all the fp out of ME interval-----
                if mm== 1
                    ...
                    index_out_me_end = find(detect_interval(:,2)<onset);
                    index_out_me_begin = find(detect_interval(:,1)>offsetInfo(MEIndex_vid(mm-1))-label2one); 
