...
for ii =1:nbSub
    ...
    for jj= 1:nbVid
        ...
        frameResult_vid = importdata([fusionPath_sub,vidIndex,'finalFrameResult',trainVecComposition,'.mat']);
...
%         size(detect_interval)
        ...
        if ~isempty(MEIndex_vid)
            ...
            for mm = 1:nb_gt
                onset_gt = onsetInfo(MEIndex_vid(mm));
                onset = onset_gt-label2one;
                offset_gt = offsetInfo(MEIndex_vid(mm));
                offset = offset_gt-label2one;
                ...
                if ~isempty(index_offset) && ~isempty(index_onset)
                    ...
                    interval2delete_fp = [interval2delete_fp,k2:k1]; % here we try to treat the interval beteen k2-k1 as one interval 
...
curretVid = curretVid+1;
    ...
    save([fusionPath,'finalCompare_',num2str(set_inteval),'_',num2str(shortLength2delete),trainVecComposition,'.mat'],'finalCompare');
    save([fusionPath,'finalCompare_me_',num2str(set_inteval),'_',num2str(shortLength2delete),trainVecComposition,'.mat'],'finalCompare_me');